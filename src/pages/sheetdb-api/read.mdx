...
<Row>
  <Col>
    ...
    The rows are counted from the same [cache](/cache) that the [Get content](#get-content) endpoint is served from, so the result may be out of date for up to the cache lifetime: the time set in your API settings when the cache is enabled, or 15 seconds when the cache is disabled.
    ...
    If you need a live count, add the `ignore_cache=1` parameter, example: [https://sheetdb.io/api/v1/58f61be4dda40/count?ignore_cache=1](https://sheetdb.io/api/v1/58f61be4dda40/count?ignore_cache=1). This request also refreshes the cache used by the [Get content](#get-content) endpoint.
    ...
    <Properties>
      ...
      <Property name="ignore_cache" type="boolean">
        Set to `1` to bypass the cache and count the rows directly from Google Sheets.
      </Property>
    </Properties>
  </Col>
  <Col>
