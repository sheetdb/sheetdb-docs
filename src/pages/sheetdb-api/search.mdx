...
<Row>
  <Col>
    ...
    If the column used in the query does not exist, you will receive a 404 error with `{"error": "Undefined column: {COLUMN_NAME}"}`. When no row matches, an empty array is returned with status 200.
